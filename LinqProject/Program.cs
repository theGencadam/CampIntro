...
namespace LinqProject
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Product> products = new List<Product>
            {
                new Product{ProductId=1,ProductName="Acer Laptop",QuantityPerUnit="32 GB Ram",UnitPrice=10000,UnitsInStock=5},
                 new Product{ProductId=1,ProductName="Acer Laptop",QuantityPerUnit="16 GB Ram",UnitPrice=8000,UnitsInStock=3},
                  new Product{ProductId=1,ProductName="Acer Laptop",QuantityPerUnit="8 GB Ram",UnitPrice=6000,UnitsInStock=2},
        ...
        {
            List<Product> filteredprducts = new List<Product>();
            foreach (var product in products)
            {
                if (product.UnitPrice > 5000 && product.UnitsInStock > 3)
                {
                    filteredprducts.Add(product);
                }

            }
            return filteredprducts;
        }
